...
namespace Stratis.Bitcoin.Features.BlockStore.Tests
{
    public class BlockRepositoryTest : LogsTestBase
    {
        ...
        public void InitializesGenBlockAndTxIndexOnFirstLoad()
        {
            ...
            using (IBlockRepository repository = this.SetupRepository(Network.BitcoinMain, dir))
            {
            }
            ...
            using (var engine = new DBreezeEngine(dir))
            {
                ...
                Assert.Equal(Network.BitcoinMain.GetGenesis().GetHash(), blockRow.Value);
        ...
        {
            string dir = CreateTestDir(this);
            Transaction trans = Network.BitcoinMain.Consensus.ConsensusFactory.CreateTransaction();
            trans.Version = 125;

            using (var engine = new DBreezeEngine(dir))
            {
                Block block = Network.BitcoinMain.Consensus.ConsensusFactory.CreateBlock();
        ...
        public void PutAsyncWritesBlocksAndTransactionsToDbAndSavesNextBlockHash()
        {
            ...
            Transaction transaction = Network.BitcoinMain.Consensus.ConsensusFactory.CreateTransaction();
            transaction.Version = 32;
            block.Transactions.Add(transaction);
            transaction = Network.BitcoinMain.Consensus.ConsensusFactory.CreateTransaction();
            transaction.Version = 48;
            block.Transactions.Add(transaction);
            blocks.Add(block);
            
            Block block2 = Network.BitcoinMain.Consensus.ConsensusFactory.CreateBlock();
            transaction = Network.BitcoinMain.Consensus.ConsensusFactory.CreateTransaction();
            ...
            using (var engine = new DBreezeEngine(dir))
            {
                ...
                foreach (KeyValuePair<byte[], byte[]> item in blockDict)
                {
                    ...
                    Assert.Equal(bl.Header.GetHash(), Block.Load(item.Value, Network.BitcoinMain).Header.GetHash());
                }
        ...
        public void GetAsyncWithExistingBlocksReturnsBlocks()
        {
            ...
            blocks[0] = Network.BitcoinMain.Consensus.ConsensusFactory.CreateBlock();
            for (int i = 1; i < blocks.Length; i++)
            {
                blocks[i] = Network.BitcoinMain.Consensus.ConsensusFactory.CreateBlock();
        ...
        public void DeleteAsyncRemovesBlocksAndTransactions()
        {
            string dir = CreateTestDir(this);
            Block block = Network.BitcoinMain.Consensus.ConsensusFactory.CreateBlock();
            block.Transactions.Add(Network.BitcoinMain.Consensus.ConsensusFactory.CreateTransaction());
