    ...
    public class FullNode : IFullNode
    {
        ...
        private void StartPeriodicLog()
        {
            this.periodicLogLoop = this.AsyncLoopFactory.Run("PeriodicLog", (cancellation) =>
            {
                ...
                {
                    var connectionsLogs = new System.Text.StringBuilder();
                    connectionsLogs.AppendLine();
                    connectionsLogs.AppendLine("======Peers======");
                    connectionsLogs.AppendLine(this.PeerAddressManager.GetPeerStats());
                    this.logger.LogInformation(connectionsLogs.ToString());
                    this.LastLogOutput = connectionsLogs.ToString();
                }
