...
namespace Stratis.Bitcoin.Utilities.JsonConverters
{
    ...
    public class MoneyInCoinsJsonConverter : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            try
            {
                return reader.TokenType == JsonToken.Null ? null : new Money((decimal)reader.Value, new MoneyUnit(100000000));
            }
            catch (InvalidCastException)
            {
        ...
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            writer.WriteValue(((Money)value).ToUnit(new MoneyUnit(100000000)));
        }
    }
}
