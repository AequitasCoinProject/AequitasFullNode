    ...
    public class MempoolPersistenceTest : IDisposable
    {
        ...
        private IEnumerable<MempoolPersistenceEntry> CreateTestEntries(int numTx)
        {
            ...
            for (int i = 0; i < numTx; i++)
            {
                ...
                var entry = new TxMempoolEntry(tx, Money.FromUnit(0.1m, Network.MoneyUnit("MilliBTC")), DateTimeOffset.Now.ToUnixTimeSeconds(), i * 100, i, amountSat, i == 0, 10, null, new PowConsensusOptions());
        ...
        {
            var amount = Money.FromUnit(satAmount, MoneyUnit.AtomicUnit);
        ...
        private static MempoolManager CreateTestMempool(NodeSettings settings, out TxMempool txMemPool)
        {
            ...
            var chain = new ConcurrentChain(Network.BitcoinMain.GetGenesis().Header);
            ...
            ConsensusRules consensusRules = new PowConsensusRules(Network.BitcoinMain, loggerFactory, dateTimeProvider, chain, new NodeDeployments(Network.BitcoinMain, chain), consensusSettings, new Checkpoints(), new InMemoryCoinView(new uint256()), new Mock<ILookaheadBlockPuller>().Object).Register(new FullNodeBuilderConsensusExtension.PowConsensusRulesRegistration());
