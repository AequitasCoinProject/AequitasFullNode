...
namespace NBitcoin.Tests
{
    public class script_tests
    {
        ...
        private void BIP65_testsCore(LockTime target, LockTime now, bool expectedResult)
        {
            ...
            Assert.Equal(expectedResult, spending.Inputs.AsIndexedInputs().First().VerifyScript(Network.BitcoinMain, tx.Outputs[0].ScriptPubKey));

            spending.Inputs[0].Sequence = uint.MaxValue;
            Assert.False(spending.Inputs.AsIndexedInputs().First().VerifyScript(Network.BitcoinMain, tx.Outputs[0].ScriptPubKey));
        }
        ...
        public void CanUseCompactVarInt()
        {
            ...
            foreach(object[] test in tests)
            {
                ...
                compact.ReadWrite(expectedBytes, Network.BitcoinMain.Consensus.ConsensusFactory);
            ...
            foreach(int i in Enumerable.Range(0, 65535 * 4))
            {
                ...
                compact.ReadWrite(bytes, Network.BitcoinMain.Consensus.ConsensusFactory);
        ...
        public void PayToMultiSigTemplateShouldAcceptNonKeyParameters()
        {
            ...
            Script redeemScript = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, tx.Inputs[1].ScriptSig).RedeemScript;
            PayToMultiSigTemplateParameters result = PayToMultiSigTemplate.Instance.ExtractScriptPubKeyParameters(Network.BitcoinMain, redeemScript);
        ...
        public void PayToPubkeyHashTemplateDoNotCrashOnInvalidSig()
        {
            ...
            PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(data));
        }
        ...
        private Script AssertCompressed(Script script, int expectedSize)
        {
            ...
            compressor.ReadWrite(compressed, Network.BitcoinMain.Consensus.ConsensusFactory);
        ...
        {
            Assert.False(TransactionSignature.IsValid(Network.BitcoinMain, new byte[0]));
            ...
            {
                Assert.True(TransactionSignature.IsValid(Network.BitcoinMain, Encoders.Hex.DecodeData(sig.ToString())));
            }
            ...
            foreach(JToken sig in sigs)
            {
                ...
                {
                    Assert.False(TransactionSignature.IsValid(Network.BitcoinMain, Encoders.Hex.DecodeData(sig.ToString())));
                }
            }
        }
        ...
        private void AssertVerifyScript(WitScript wit, Money amount, Script scriptSig, Script scriptPubKey, ScriptVerify flags, int testIndex, string comment, ScriptError expectedError)
        {
            ...
            ScriptError actual;
            Script.VerifyScript(Network.BitcoinMain, scriptSig, scriptPubKey, spendingTransaction, 0, amount, flags, SigHash.Undefined, out actual);
        ...
        {
            uint256 hash = Script.SignatureHash(Network.BitcoinMain, scriptPubKey, transaction, 0, SigHash.All);
        ...
        {
            Assert.False(Script.VerifyScript(Network.BitcoinMain, scriptPubKey, tx, n, null, this.flags));
#if !NOCONSENSUSLIB
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
        ...
        {
            Assert.True(Script.VerifyScript(Network.BitcoinMain, scriptPubKey, tx, n, null, this.flags));
#if !NOCONSENSUSLIB
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
        ...
        public void script_single_hashtype()
        {
            ...
            Assert.True(tx.Inputs.AsIndexedInputs().ToArray()[2].VerifyScript(Network.BitcoinMain, scriptPubKey, out ScriptError error));
        }
        ...
        public void script_combineSigs()
        {
            ...
            Script combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, empty, empty);
            ...
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, scriptSig, empty);
            Assert.True(combined == scriptSig);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, empty, scriptSig);
            Assert.True(combined == scriptSig);
            ...
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, scriptSigCopy, scriptSig);
            ...
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, scriptSigCopy, scriptSig);
            Assert.True(combined == scriptSig);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, scriptSig, scriptSigCopy);
            Assert.True(combined == scriptSig);
            ...
            uint256 hash1 = Script.SignatureHash(Network.BitcoinMain, scriptPubKey, txTo, 0, SigHash.All);
            var sig1 = new TransactionSignature(keys[0].Sign(hash1), SigHash.All);

            uint256 hash2 = Script.SignatureHash(Network.BitcoinMain, scriptPubKey, txTo, 0, SigHash.None);
            var sig2 = new TransactionSignature(keys[1].Sign(hash2), SigHash.None);


            uint256 hash3 = Script.SignatureHash(Network.BitcoinMain, scriptPubKey, txTo, 0, SigHash.Single);
            ...
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial1a, partial1b);
            Assert.True(combined == partial1a);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial1a, partial2a);
            Assert.True(combined == complete12);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial2a, partial1a);
            Assert.True(combined == complete12);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial1b, partial2b);
            Assert.True(combined == complete12);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial3b, partial1b);
            Assert.True(combined == complete13);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial2a, partial3a);
            Assert.True(combined == complete23);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial3b, partial2b);
            Assert.True(combined == complete23);
            combined = Script.CombineSignatures(Network.BitcoinMain, scriptPubKey, txTo, 0, partial3b, partial3a);
        ...
        public void script_PushData()
        {
            ...
            var context = new ScriptEvaluationContext(Network.BitcoinMain)
            {
        ...
        public void CanParseAndGeneratePayToPubKeyScript()
        {
            ...
            PayToPubkeyHashScriptSigParameters sigResult = PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, scriptSig);
            ...
            sigResult = PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, scriptSig);
        ...
        public void CanParseAndGeneratePayToPubkey()
        {
            ...
            TransactionSignature sig = PayToPubkeyTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            Assert.NotNull(sig);
            Assert.True(PayToPubkeyTemplate.Instance.CheckScriptSig(Network.BitcoinMain, new Script(scriptSig), null));
            ...
            sig = PayToPubkeyTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            Assert.Null(sig);
            Assert.False(PayToPubkeyTemplate.Instance.CheckScriptSig(Network.BitcoinMain, new Script(scriptSig), null));
        ...
        public void CanParseAndGenerateSegwitScripts()
        {
            ...
            Assert.Equal(scriptPubKey, PayToWitPubKeyHashTemplate.Instance.GenerateScriptPubKey((BitcoinWitPubKeyAddress)pubkey.WitHash.GetAddress(Network.BitcoinMain)));
        ...
        public void CanParseAndGeneratePayToMultiSig()
        {
            ...
            PayToMultiSigTemplateParameters scriptPubKeyResult = PayToMultiSigTemplate.Instance.ExtractScriptPubKeyParameters(Network.BitcoinMain, new Script(scriptPubKey));
            ...
            TransactionSignature[] result = PayToMultiSigTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            ...
            result = PayToMultiSigTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            ...
            Script actual = PayToScriptHashTemplate.Instance.GenerateScriptSig(Network.BitcoinMain, new[] { sig, sig }, new Script(scriptPubKey));
        ...
        public void CanExtractAddressesFromScript()
        {
            ...
            Assert.Null(payToMultiSig.GetSigner(Network.BitcoinMain));
            PubKey[] destinations = payToMultiSig.GetDestinationPublicKeys(Network.BitcoinMain);
            ...
            Assert.NotNull(payToScriptHash.GetDestination(Network.BitcoinMain));
            Assert.IsType<ScriptId>(payToScriptHash.GetDestination(Network.BitcoinMain));
            Assert.Equal("b5b88dd9befc9236915fcdbb7fd50052df50c855", payToScriptHash.GetDestination(Network.BitcoinMain).ToString());
            Assert.True(payToScriptHash.GetDestination(Network.BitcoinMain).GetAddress(Network.BitcoinMain).GetType() == typeof(BitcoinScriptAddress));

            var payToPubKeyHash = new Script("OP_DUP OP_HASH160 356facdac5f5bcae995d13e667bb5864fd1e7d59 OP_EQUALVERIFY OP_CHECKSIG");
            Assert.NotNull(payToPubKeyHash.GetDestination(Network.BitcoinMain));
            Assert.IsType<KeyId>(payToPubKeyHash.GetDestination(Network.BitcoinMain));
            Assert.Equal("356facdac5f5bcae995d13e667bb5864fd1e7d59", payToPubKeyHash.GetDestination(Network.BitcoinMain).ToString());
            Assert.True(payToPubKeyHash.GetDestination(Network.BitcoinMain).GetAddress(Network.BitcoinMain).GetType() == typeof(BitcoinPubKeyAddress));
            ...
            Assert.NotNull(p2shScriptSig.GetSigner(Network.BitcoinMain));
            Assert.IsType<ScriptId>(p2shScriptSig.GetSigner(Network.BitcoinMain));
            Assert.Equal("b5b88dd9befc9236915fcdbb7fd50052df50c855", p2shScriptSig.GetSigner(Network.BitcoinMain).ToString());
            ...
            Assert.NotNull(p2phScriptSig.GetSigner(Network.BitcoinMain));
            Assert.IsType<KeyId>(p2phScriptSig.GetSigner(Network.BitcoinMain));
            Assert.Equal("352183abbcc80a0cd7c051a28df0abbf1e80ac3e", p2phScriptSig.GetSigner(Network.BitcoinMain).ToString());
        }
        ...
        public void P2PKHScriptSigShouldNotBeMistakenForP2SHScriptSig()
        {
            ...
            PayToScriptHashSigParameters result = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, p2pkhScriptSig);
        ...
        public void CanParseAndGeneratePayToScript()
        {
            ...
            PayToScriptHashSigParameters sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            Assert.Equal("3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301", Encoders.Hex.EncodeData(sigParams.GetMultisigSignatures(Network.BitcoinMain)[0].ToBytes()));
            ...
            sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig), sigParams.RedeemScript.PaymentScript);
            Assert.NotNull(sigParams);
            sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig), new Script("OP_HASH160 b5b88dd9befc9236915fcdbb7fd50052df50c853 OP_EQUAL"));
            Assert.Null(sigParams);
            sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig), new Script("OP_HASH160 OP_EQUAL"));
            Assert.Null(sigParams);
            ///
            ...
            sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(Network.BitcoinMain, new Script(scriptSig));
            Assert.Null(sigParams.GetMultisigSignatures(Network.BitcoinMain)[0]);
