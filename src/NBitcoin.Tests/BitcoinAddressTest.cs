...
namespace NBitcoin.Tests
{
    public class BitcoinAddressTest
    {
        ...
        {
            String key = "";
            Assert.Throws<FormatException>(() => BitcoinAddress.Create(key, Network.BitcoinMain));

            key = null;
            Assert.Throws<ArgumentNullException>(() => BitcoinAddress.Create(key, Network.BitcoinMain));
        }
    }
}
