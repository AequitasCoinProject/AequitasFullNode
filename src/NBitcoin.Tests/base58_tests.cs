    ...
    public class base58_tests
    {
            ...
            foreach(TestCase test in tests)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        Assert.True(dest.ScriptPubKey.GetDestination(Network.BitcoinMain) == dest);
                    }
        ...
        public void base58_keys_invalid()
        {
            foreach(object[] i in InvalidKeys)
            {
                ...
                Assert.Throws<FormatException>(() => Network.BitcoinMain.CreateBitcoinAddress(data));
                Assert.Throws<FormatException>(() => Network.BitcoinMain.CreateBitcoinSecret(data));
            }
        }
    }
}
