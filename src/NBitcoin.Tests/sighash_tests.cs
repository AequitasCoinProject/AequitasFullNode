    ...
    public class sighash_tests
    {
        ...
        public void sighash_from_data()
        {
            ...
            foreach(TestCase test in tests)
            {
                ...
                tx.ReadWrite(ParseHex(raw_tx), Network.BitcoinMain.Consensus.ConsensusFactory);

                byte[] raw = ParseHex(raw_script);
                scriptCode = new Script(raw);

                uint256 sh = Script.SignatureHash(Network.BitcoinMain, scriptCode, tx, nIn, (SigHash)nHashType);
