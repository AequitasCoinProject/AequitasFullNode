...
namespace NBitcoin
{
    ...
    public class BitcoinNetwork : Network
    {
        ...
        public static int BitcoinMaxTimeOffsetSeconds = 70 * 60;

        /// <summary> Bitcoin default value for the maximum tip age in seconds to consider the node in initial block download (2 hours). </summary>
        public static int BitcoinDefaultMaxTipAgeInSeconds = 24 * 60 * 60;

        public Network Init(NetworkInitializationMode mode)
        {
            this.CoinName = "Bitcoin";            
            this.RootFolderName = "bitcoin";
            this.DefaultConfigFilename = "bitcoin.conf";
        ...
        private Network InitMain()
        {
            ...
            this.consensus.DefaultAssumeValid = new uint256("0x00000000000000000086a2362d78ffb4e41dc0b8775ff3c278bd994a5162348a"); // 499610

            // The message start string is designed to be unlikely to occur in normal data.
            // The characters are rarely used upper ASCII, not valid as UTF-8, and produce
            // a large 4-byte int at any alignment.
            this.magic = 0xD9B4BEF9;
            this.alertPubKeyArray = Encoders.Hex.DecodeData("04fc9702847840aaf195de8442ebecedf5b095cdbb9bc716bda9110971b28a49e0ead8564ff0db22209e0374782c093bb899692d524e9d6a6956e7c5ecbcd68284");
            ...
            this.seeds.Add(new DNSSeedData("bitcoin.sipa.be", "seed.bitcoin.sipa.be")); // Pieter Wuille
            this.seeds.Add(new DNSSeedData("bluematt.me", "dnsseed.bluematt.me")); // Matt Corallo
            this.seeds.Add(new DNSSeedData("dashjr.org", "dnsseed.bitcoin.dashjr.org")); // Luke Dashjr
            this.seeds.Add(new DNSSeedData("bitcoinstats.com", "seed.bitcoinstats.com")); // Christian Decker
            this.seeds.Add(new DNSSeedData("xf2.org", "bitseed.xf2.org")); // Jeff Garzik
            this.seeds.Add(new DNSSeedData("bitcoin.jonasschnelli.ch", "seed.bitcoin.jonasschnelli.ch")); // Jonas Schnelli
            ...
            for (int i = 0; i < pnSeed.Length; i++)
            {
                ...
                addr.Endpoint = Utils.ParseIpEndpoint(pnSeed[i], this.DefaultPort);
                this.fixedSeeds.Add(addr);
            }
        ...
        private Network InitTest()
        {
            ...
            this.alertPubKeyArray = DataEncoders.Encoders.Hex.DecodeData("04302390343f91cc401d56d68b123028bf52e5fca1939df127f63c6467cdf9c8e2c14b61104cf817d0b780da337893ecc4aaff1309e536162dabbdb45200ca2b0a");
            ...
            this.fixedSeeds.Clear();
            this.seeds.Clear();
            this.seeds.Add(new DNSSeedData("bitcoin.petertodd.org", "testnet-seed.bitcoin.petertodd.org"));
            this.seeds.Add(new DNSSeedData("bluematt.me", "testnet-seed.bluematt.me"));
            this.seeds.Add(new DNSSeedData("bitcoin.schildbach.de", "testnet-seed.bitcoin.schildbach.de"));

            this.base58Prefixes = Network.BitcoinMain.base58Prefixes.ToArray();
        ...
        private Network InitReg()
        {
            ...
            this.seeds.Clear();  // Regtest mode doesn't have any DNS seeds.
            this.base58Prefixes = Network.BitcoinTest.base58Prefixes.ToArray();
            this.base58Prefixes[(int)Base58Type.PUBKEY_ADDRESS] = new byte[] { (111) };
            this.base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS] = new byte[] { (196) };
            this.base58Prefixes[(int)Base58Type.SECRET_KEY] = new byte[] { (239) };
            this.base58Prefixes[(int)Base58Type.EXT_PUBLIC_KEY] = new byte[] { (0x04), (0x35), (0x87), (0xCF) };
